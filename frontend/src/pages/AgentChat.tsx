...
// Define the missing ToolCall type
interface ToolCall {
    id: string;
    function: {
        name: string;
...
export default function AgentChat() {
    ...
    const sendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!input.trim() || loading || !thinkAgent) return;

        setLoading(true);
        setShouldAutoScroll(true); // Always scroll on send
        try {
            await thinkAgent.sendMessageAsync(input, {});
            setInput('');
            setThinking(true);
            scrollToBottom();
        } catch (error) {
                    ...
                    <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            ...
                            <div className="flex items-center space-x-2">
                                <Avatar role="assistant" name={thinkAgent.name} size="sm" />
                                <h1 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
                                    <span>{thinkAgent.name}</span>
                                    {thinking && (
                    ...
                    {messages.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center py-10">
                            ...
                            <div className="text-gray-500 max-w-md mb-8">
                                Type your message below to begin chatting with {thinkAgent.name}
                            </div>
                        ...
                        <>
                            ...
                            {/* Typing indicator */}
                            {thinking && <TypingIndicator agentName={thinkAgent.name} />}
                    ...
                    <form onSubmit={sendMessage} className="max-w-4xl mx-auto">
                        <div className="relative flex">
                            <input
                                ...
                                placeholder={`Message ${thinkAgent.name}...`}
